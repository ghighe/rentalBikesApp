...
rentals.get("/getRevenueRentals", (req, res) => {
  ...
  let last_month = functions.transformDate(
    new Date(
  ...
  database.query(
    `SELECT SUM(invoice.net_amount) AS total_net_amount FROM rentals LEFT JOIN invoice ON invoice.id = rentals.invoice_id WHERE rentals.end_date_time <= "${current_date}" AND rentals.start_date_time BETWEEN "${last_month}" AND "${current_date}"`,
    (err, result, fields) => {
      if (err) {
        res.json({
          type: "error",
      ...
      if (result.length != 0) {
        res.json({
          type: "success",
          message: { total_net_amount: result[0].total_net_amount }
        });
      } else {
...
rentals.get("/getBikesCount", (req, res) => {
  database.query(
